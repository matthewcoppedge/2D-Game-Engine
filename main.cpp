...
GLScene*scene = new GLScene; //Create scene class
							...
							LPARAM	lParam)			// Additional Message Information
{
	...
	switch (uMsg)									// Check For Windows Messages
	{
		...
		case WM_SIZE:								// Resize The OpenGL Window
		{
			...
			scene->resizeScene(LOWORD(lParam), HIWORD(lParam));      //Allows scene to resize
...
//	if (MessageBox(NULL,"Would You Like To Run In Fullscreen Mode?", "Start FullScreen?",MB_YESNO|MB_ICONQUESTION)==IDNO)
	...
	// Check if wglSwapIntervalEXT is available
    typedef BOOL (APIENTRY * PFNWGLSWAPINTERVALEXTPROC)(int interval);
    PFNWGLSWAPINTERVALEXTPROC wglSwapIntervalEXT = (PFNWGLSWAPINTERVALEXTPROC) wglGetProcAddress("wglSwapIntervalEXT");

    HANDLE hProcess = GetCurrentProcess();

    // Set the affinity mask for two cores (e.g., cores 0 and 1)
    // Affinity mask uses binary, so 0b00000011 means cores 0 and 1
    //DWORD_PTR dwAffinityMask = 0b00000011;


	// Create Our OpenGL Window
	if (!CreateGLWindow("Arcane",fullscreenWidth,fullscreenHeight,256,fullscreen))
	{
	...
	delete scene;                                   // Delete scene before exiting program
